...
import Distribution.Simple.Program.HcPkg (hcPkgProgram)
...
import Distribution.Simple.GHC.Props ( GhcImplProps, getImplProps )
...
build pkg_descr lbi flags suffixes = do
  ...
  let Just hcPkgProg =
        lookupProgram (hcPkgProgram $ compilerFlavor (compiler lbi))
                      (withPrograms lbi)
...
createInternalPackageDB :: Verbosity -> GhcImplProps -> ConfiguredProgram
                        -> FilePath -> IO PackageDB
createInternalPackageDB verbosity props hcPkgProg distPref = do
    ...
    HcPkg.init verbosity props hcPkgProg dbDir
    return packageDB
