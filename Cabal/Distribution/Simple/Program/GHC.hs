...
import Distribution.Simple.GHC.Props ( getImplProps, GhcImplProps(..) )
...
--     Just GhcModeEvaluate    -> ["-e", expr]
  ...
  , [ "-#include \"" ++ inc ++ "\""
    | inc <- flags ghcOptFfiIncludes, flagFfiIncludes props ]
  ...
  , packageDbArgs props (ghcOptPackageDBs opts)
...
packageDbArgs :: GhcImplProps -> PackageDBStack -> [String]
